...
export class AnalyticsFallbackService {
  ...
  private buildExecutiveSummary(context: AnalyticsContextPayload): string {
  ...
  private buildTrends(context: AnalyticsContextPayload): string[] {
    ...
    if (trends.length < 3) {
      trends.push(
        `Среднегодовое изменение численности населения за период составляет ${this.formatPercent(context.monitoring.averageAnnualChangePercent)}.`,
      );
    }
  ...
  private buildForecastAssessment(context: AnalyticsContextPayload): string {
  ...
  private buildPolicyRecommendations(context: AnalyticsContextPayload): string[] {
    ...
    if ((latest.naturalGrowth ?? recent.naturalGrowth ?? 0) < 0) {
      recommendations.push(
        'Сфокусировать социальную политику на снижении естественной убыли: расширить меры поддержки семей с детьми, повысить доступность первичной медико-санитарной помощи и профилактических программ.',
      );
    }

    if ((latest.migrationRate ?? recent.migrationRate ?? 0) < 0) {
      recommendations.push(
        'Запустить адресные меры по удержанию и привлечению населения: поддержка рабочих мест, повышение доступности жилья и развитие повседневной транспортной связанности.',
      );
    }

    if (context.monitoring.periodChangePercent > 0 || context.forecast.changePercent > 0) {
      recommendations.push(
        'Подготовить опережающее расширение мощностей школ, детских садов, поликлиник и социальной инфраструктуры в зонах потенциального роста населения.',
      );
    } else {
      recommendations.push(
        'Провести ревизию сети социальных учреждений и перейти к более гибким форматам обслуживания там, где население сокращается или стареет.',
      );
    }

    if (context.entity.level === 'municipality') {
      recommendations.push(
        'Синхронизировать муниципальные меры поддержки с региональными программами занятости, здравоохранения и семейной политики для концентрации ресурсов на наиболее уязвимых группах.',
      );
    } else {
      recommendations.push(
        'Дифференцировать меры по муниципалитетам региона: усиливать поддержку территорий с устойчивой убылью и закреплять точки роста в муниципалитетах с положительной миграционной динамикой.',
      );
    }

    recommendations.push(
      'Регулярно обновлять мониторинг демографических коэффициентов и оценивать результативность принятых мер не реже одного раза в год.',
    );

    return this.unique(recommendations).slice(0, 6);
  }

  private buildPlanningRecommendations(context: AnalyticsContextPayload): string[] {
    const recommendations: string[] = [];

    if (context.forecast.changePercent > 0) {
      recommendations.push(
        'Зарезервировать территории под жилищное строительство и развитие инженерной инфраструктуры в наиболее привлекательных для проживания зонах.',
      );
      recommendations.push(
        'Уточнить параметры транспортной и социальной обеспеченности новых и растущих жилых массивов с учетом прогнозного увеличения нагрузки.',
      );
    } else {
      recommendations.push(
        'Скорректировать территориальное планирование под сценарий стагнации или снижения численности: ограничить избыточное расширение застройки и приоритизировать модернизацию уже освоенных территорий.',
      );
      recommendations.push(
        'Рассмотреть адаптивное использование объектов и площадок в территориях с устойчивым сокращением населения, чтобы снизить нагрузку на бюджет содержания инфраструктуры.',
      );
    }

    recommendations.push(
      'При размещении социальных объектов учитывать внутритерриториальные различия по муниципалитетам и прогнозной концентрации населения, а не только текущую численность.',
    );

    if (context.entity.level === 'municipality') {
      recommendations.push(
        'Увязать муниципальные решения по землепользованию, общественному транспорту и размещению услуг с ролью территории в системе расселения региона.',
      );
    } else {
      recommendations.push(
        'Использовать прогноз для уточнения схемы расселения: поддерживать опорные центры роста и усиливать межмуниципальную связанность периферийных территорий.',
      );
    }
